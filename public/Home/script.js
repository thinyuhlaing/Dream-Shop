...
const run = () => {
  ...
  const itemvalue = items.filter((product) => {
  ...
  if (itemvalue.length > 0) {
    for (let i = 0; i < itemvalue.length; i++) {
      ...
      imageTag.src = itemvalue[i].images[0];

      const detailTag = document.createElement("div");
      detailTag.classList.add("detail");

      const titleTag = document.createElement("div");
      titleTag.classList.add("title");
      titleTag.innerHTML = itemvalue[i].title;

      const descriptionTag = document.createElement("div");
      descriptionTag.classList.add("descri");
      descriptionTag.innerHTML = itemvalue[i].description;

      const priceTag = document.createElement("div");
      priceTag.classList.add("price");
      priceTag.innerHTML = "$" + itemvalue[i].price;

      const discountTag = document.createElement("div");
      discountTag.classList.add("disco");
      discountTag.innerHTML = "-" + itemvalue[i].discountPercentage + "%";
      ...
      boxTag.append(imageTag, detailTag);
      middle.append(boxTag);
    }
  } else {
